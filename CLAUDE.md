...
Deployed to two independent places — they don't sync with each other (separate domains, separate `localStorage`):

- **Firebase Hosting** (project ID `touch-type-trainer`, config in `firebase.json`/`.firebaserc`), live at https://touch-type-trainer.web.app. Firebase CLI is authenticated as dennishungnguyen@gmail.com and reachable from Claude's Bash tool on this machine — deploy with `npx firebase-tools deploy --only hosting` from this directory.
- **GitHub Pages**, live at https://therealmenace10040.github.io/touch-type-trainer/. Repo is `TheRealMenace10040/touch-type-trainer` (`gh` CLI at `C:\Program Files\GitHub CLI\gh.exe`, not on PATH by default). Deploys automatically via `.github/workflows/deploy-pages.yml` on every push to `main` — just `git push`, no manual Pages step.

Workflow (same pattern used across Dennis's other projects): make the edit, verify it locally first, then ask before deploying — don't auto-deploy after every change. When deploying, do both targets (`firebase deploy --only hosting` and `git push`) so they don't drift out of sync with each other.
...
- **`public/js/keyboard.js`** — Renders the on-screen keyboard (`buildKeyboard`) and manages visual state: `setNextKeys([...])` highlights the key(s) to press next (a letter plus, for capitals, the required Shift key), `flashKey(key, className)` gives brief correct/error feedback. `ROWS` is a full physical-keyboard layout (5 rows, variable key widths in `u` units — Backspace 2u, Tab 1.5u, Enter 2.25u, Space 10u, etc.), including decorative modifier keys (Tab/Caps/Enter/Backspace/Ctrl/Alt) that exist only for visual completeness — they're never lesson targets and `typing.js` already ignores their physical-keyboard equivalents, so adding more decorative keys never needs a `typing.js` change. Finger colors are applied as a `--finger-color` CSS custom property per key, consumed by `css/style.css`.
...
### Styling — the `--finger-color` hook

`public/css/style.css` is a dark "Signal" theme (oklch colors, Chakra Petch/IBM Plex fonts loaded via Google Fonts `<link>` tags in `index.html`'s `<head>`). The finger color-coding is *not* hardcoded per-key in CSS — `keyboard.js` sets a `--finger-color` custom property inline on each key element (`keyEl.style.setProperty('--finger-color', FM.FINGER_COLORS[finger])`), and `.key.key-dot::after` in `style.css` consumes it to render a small colored dot at the bottom-center of true letter/number/symbol keys (`.key-dot` is only added for keys resolved via `KEY_TO_FINGER`, not Space/Shift/decorative keys — matching the current design's spec of "no dot on modifiers"). Any future restyle must keep reading `var(--finger-color)` somewhere in `.key` — it's easy to drop this by accident when swapping in a new theme (a design review file provided mid-project did exactly that once already, shipping flat single-color keys) since nothing else in the app breaks if it's missing, it just silently loses the per-finger teaching cue.

Key sizing is unit-based, not literal pixels: `.keyboard` defines `--key-unit` (responsive via `clamp()`) and `--key-gap`, and each `.key` sets `--u` inline (its width in key-units from `keyboard.js`'s `ROWS`) — width is computed as `calc(var(--key-unit) * var(--u, 1) + (var(--u, 1) - 1) * var(--key-gap))`. This lets wide keys (Space, Enter, Shift) size correctly relative to normal 1u keys while staying responsive; row-to-row physical stagger comes from explicit `margin-left` on `.kb-row-1/2/3` (flex `justify-content: center` means natural key-width differences alone don't produce the stagger look, since centering cancels it out).

**Gotcha discovered while building this:** rapidly toggling a key's state classes (`next-key`/`key-correct`/`key-error`) via direct synchronous JS (e.g. `el.classList.add(...)` immediately followed by a same-tick `getComputedStyle()` read, repeated many times back-to-back without letting the browser actually paint in between) can leave `getComputedStyle()` reporting stale pre-transition color values, because `.key` has `transition: background 0.15s, ...`. This only showed up under synthetic rapid-fire testing, not real usage (real keydown events are naturally spaced out) — but if a future automated test flakes on key-state colors, suspect this before assuming the CSS/JS logic is wrong; a real fresh page load + realistic timing (or `transition: none` during the test) resolves it.
